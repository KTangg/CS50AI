...
def load_data(filename):
    ...
    with open(filename) as f:
        ...
        # Skipping head
        next(reader)
        Months_dict = {"jan": 0, "feb": 1, "mar": 2, "apr": 3, "may": 4, "june": 5, "jul": 6, "aug": 7, "sep": 8, "oct": 9, "nov": 10, "dec": 11}
        for row in reader:
            # Getting Value
            ...
            Month = int(Months_dict[row[10].lower()])
            OperatingSystems = int(row[11])
            Browser = int(row[12])
            Region = int(row[13])
            TrafficType = int(row[14])
            VisitorType = int(1 if row[15].lower() == "returning" else 0)
            Weekend = int(1 if row[16].lower() == "true" else 0)

            # Creating list and add to evidences
            evidence = [Administrative, Administrative_Duration, Informational, Informational_Duration, ProductRelated, ProductRelated_Duration, 
                    BounceRates, ExitRates, PageValues, SpecialDay, Month, OperatingSystems, Browser, Region, TrafficType, VisitorType, Weekend]

            evidences.append(evidence)

            # Add Labels list
            Revenue = int(1 if row[17].lower() == "true" else 0)
