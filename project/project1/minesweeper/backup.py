...
class MinesweeperAI():
    ...
    def add_knowledge(self, cell, count):
        ...
        if len(self.knowledge) == 0:
            ...
            # New temp Sentence
            add_sentence = Sentence(set(), None)
            # Loop for all sentence in knowledge check for subset
            for sentence in self.knowledge:
                # Self Exclude & set()
                if sentence is not new_sentence and sentence.cells != set() and new_sentence.cells != set():

                    if new_sentence.cells.issubset(sentence.cells):
                        # Substract
                        print(f"{new_sentence} is subset of {sentence}")
                        add_sentence.cells = sentence.cells - new_sentence.cells
                        add_sentence.count = sentence.count - new_sentence.count
                        print(f"Remove {sentence} Insert {add_sentence}")
                        self.knowledge.remove(sentence)
                        self.knowledge.append(add_sentence)

                        # Check for new mine or space
                        self.mark_new(add_sentence)


                    elif sentence.cells.issubset(new_sentence.cells):
                        # Substract
                        print(f"{sentence} is subset of {new_sentence}")
                        add_sentence.cells = new_sentence.cells - sentence.cells
                        add_sentence.count = new_sentence.count - sentence.count
                        print(f"Remove {new_sentence} Insert {add_sentence}")
        ...
        if blank_sentence in self.knowledge:
            self.knowledge.remove(blank_sentence)
